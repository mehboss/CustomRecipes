...
/**
 * <b>XItemStack</b> - YAML <a href=
 * "https://hub.spigotmc.org/javadocs/spigot/org/bukkit/inventory/ItemStack.html">ItemStack</a>
 * Serializer<br>
 ...
 * }</pre>
 * <p>
 * What's the point of this class when
 * {@link org.bukkit.configuration.MemorySection#getItemStack(String)} exists?
 * That method works based on YAML tags which makes the config hideous and
 * doesn't have syntax sugars for certain configurations to make the config
 * cleaner and concise. Also, certain values will have unreadable formats.
 *
 * @author Crypto Morin
 * @version 9.0.0
 * @see XMaterial
...
public final class XItemStack {
	 ...
	 * 
	 * @deprecated You should now use {@link Deserializer#deserialize()} which
	 *             handles {@link UnsetMaterialCondition}.
	 */
	@Deprecated
	...
	private static <M extends ItemMeta> void meta(Class<? extends M> clazz,
			Function<Deserializer, MetaHandler<M>> deserialize, Function<Serializer, MetaHandler<M>> serialize) {
		DESERIALIZE_META_HANDLERS.put(clazz, Optional.of(cast(deserialize)));
		SERIALIZE_META_HANDLERS.put(clazz, Optional.of(cast(serialize)));
	}
	...
	private static <T extends SerialObject> void recursiveMetaHandle(T serialObject, Class<?> metaClass, ItemMeta meta,
		...
		if (superclass != null)
			recursiveMetaHandle(serialObject, superclass, meta, map, subCollectedHandlers);
	...
	private static BlockState safeBlockState(BlockStateMeta meta) {
		try {
			...
			// net.minecraft.server.v1_9_R2.TileEntityDispenser cannot be cast to
			// net.minecraft.server.v1_9_R2.TileEntityDropper
			return null;
		}
	}
	...
	public static void serialize(@NotNull ItemStack item, @NotNull ConfigurationSection config,
		...
		new Serializer().fromItem(item).toConfig(config).withTranslator(translator).serialize();
	}
	...
	@Deprecated
	public static ItemStack deserialize(@NotNull ConfigurationSection config,
			@NotNull Function<String, String> translator, @Nullable Consumer<Exception> restart) {
	...
	/**
	 * Get a new instance of {@link Serializer} used for serializing items into a
	 * configuration.
	 */
	...
	public static final class Serializer extends SerialObject {
		...
		public Serializer copySettings() {
			return new Serializer().toConfig(config).withTranslator(translator).withMiniMessage(miniMessageHandler);
		}
		...
		private void customModelData() {
			...
			if (SUPPORTS_CUSTOM_MODEL_DATA) {
				if (SUPPORTS_ADVANCED_CUSTOM_MODEL_DATA) {
					...
					if (idCount == 1) {
						if (!strings.isEmpty())
							config.set("custom-model-data", singleOrList(strings));
						if (!floats.isEmpty())
							config.set("custom-model-data", singleOrList(floats));
						if (!flags.isEmpty())
							config.set("custom-model-data", singleOrList(flags));
						if (!colors.isEmpty())
							config.set("custom-model-data", singleOrList(
									colors.stream().map(Serializer::colorString).collect(Collectors.toList())));
					} else {
						ConfigurationSection cfgCustomModelData = config.createSection("custom-model-data");
						if (!strings.isEmpty())
							cfgCustomModelData.set("strings", strings);
						if (!floats.isEmpty())
							cfgCustomModelData.set("floats", floats);
						if (!flags.isEmpty())
							cfgCustomModelData.set("flags", flags);
						if (!colors.isEmpty())
							cfgCustomModelData.set("colors",
									colors.stream().map(Serializer::colorString).collect(Collectors.toList()));
					}
		...
		private void handleSuspiciousStewMeta(SuspiciousStewMeta stew) {
			...
			for (PotionEffect effect : customEffects) {
				effects.add(XPotion.of(effect.getType()).name() + ", " + effect.getDuration() + ", "
						+ effect.getAmplifier());
			}
		...
		private void handleCrossbowMeta(CrossbowMeta crossbow) {
			...
			for (ItemStack projectiles : crossbow.getChargedProjectiles()) {
				this.copySettings().fromItem(projectiles).toConfig(config.getConfigurationSection("projectiles." + i))
						.serialize();
				i++;
			}
		}
		...
		private void handleArmorMeta(ArmorMeta armorMeta) {
			if (armorMeta.hasTrim()) {
				...
				trimConfig.set("material",
						trim.getMaterial().getKey().getNamespace() + ':' + trim.getMaterial().getKey().getKey());
				trimConfig.set("pattern",
						trim.getPattern().getKey().getNamespace() + ':' + trim.getPattern().getKey().getKey());
			}
		}
		...
		private void handleBookMeta(BookMeta book) {
			if (book.getTitle() != null || book.getAuthor() != null || book.getGeneration() != null
					|| !book.getPages().isEmpty()) {
		...
		private static final boolean SUPPORTS_PotionMeta_getBasePotionType = XReflection.of(PotionMeta.class)
				.method("public org.bukkit.potion.PotionType getBasePotionType()").exists();
		private static final boolean SUPPORTS_PotionEffectType_getKey = XReflection.of(PotionEffectType.class)
				.method("public org.bukkit.NamespacedKey getKey()").exists();
		...
		private void handlePotionMeta(PotionMeta meta) {
			if (supports(9)) {
				...
				if (SUPPORTS_POTION_COLOR && meta.hasColor())
					config.set("color", meta.getColor().asRGB());
			} else {
				...
				// config.set("base-effect", potion.getType().name() + ", " +
				// potion.hasExtendedDuration() + ", " + potion.isSplash());
				// }
			}
		}
		...
		private void handleBlockStateMeta(BlockStateMeta meta) {
			...
			if (supports(11) && state instanceof ShulkerBox) {
				...
				for (ItemStack itemInBox : box.getInventory().getContents()) {
					if (itemInBox != null) {
						this.copySettings().fromItem(itemInBox).toConfig(shulker.createSection(Integer.toString(i)))
								.serialize();
					}
					i++;
				}
	...
	/**
	 * Get a new instance of {@link Deserializer} used for deserializing items from
	 * a configuration.
	 */
	...
	/**
	 * The deserializer only requires a config to be set. All other properties are
	 * optional. To use this class, you have to use
	 * {@link XItemStack#deserializer()}. This class is not thread-safe.
	 */
	public static final class Deserializer extends SerialObject {
		...
		/**
		 * If an item is set, it'll modify this item instead of making a new one from
		 * the config.
		 */
		...
		/**
		 * Used for {@link ItemMeta#getDisplayName()} and {@link ItemMeta#getLore()} if
		 * your plugin handles them in a specific way. This utility class does not
		 * colorize any strings by default. You can also set a
		 * {@link #withMiniMessage(Function)} for Paper servers.
		 */
		...
		/**
		 * Restarts are a way to handle {@link Exception} without terminating the
		 * process in general by providing a solution on the fly. If you don't set a
		 * restart, the deserialization will fail entirely with the exception.
		 * <p>
		 * Currently the only properly handled exceptions that support restarts are
		 * {@link MaterialCondition} any other exception will not trigger a restart.
		 */
		...
		/**
		 * If this server implementation supports adventure API,
		 * {@link ItemMeta#getDisplayName()} and {@link ItemMeta#getLore()} will be set
		 * using their respective Paper adventure API. If the adventure API
		 * implementation is not available, {@link #withTranslator(Function)} is used
		 * instead.
		 */
		@ApiStatus.Experimental
		...
		public Deserializer copySettings() {
			return new Deserializer().fromConfig(config).withTranslator(translator).withRestart(restart)
		...
		private void attributes() {
			...
			if (attributes != null) {
				for (String attribute : attributes.getKeys(false)) {
					...
					if (!attributeInst.isPresent() || !attributeInst.get().isSupported())
						continue;
					...
					AttributeModifier modifier = XAttribute.createModifier(attrName, section.getDouble("amount"),
		...
		private void customModelData() {
			...
			if (SUPPORTS_ADVANCED_CUSTOM_MODEL_DATA) {
				...
				if (detailed != null) {
					...
					customModelData.setColors(
							parseRawOrList("color", "colors", detailed, x -> XItemStack.parseColor(x).orElseThrow(
									() -> new IllegalArgumentException("Unknown color for custom model data: " + x))));
				} else {
					...
					if (!listed.isEmpty()) {
						...
						if (modelData != null && !modelData.isEmpty()) {
							if (tryNumber(modelData, Float::parseFloat) != null) {
								customModelData
										.setFloats(listed.stream().map(Float::parseFloat).collect(Collectors.toList()));
							} else {
								Optional<Color> color = parseColor(modelData);
								if (color.isPresent()) {
									customModelData.setColors(listed.stream().map(XItemStack::parseColor)
											.map(x -> x.orElseThrow(() -> new IllegalArgumentException(
													"Unknown color for custom model data: " + x)))
											.collect(Collectors.toList()));
								} else {
				...
				// Setting an empty component will save it and change the internal meta which
				// affects isSimilar()
		...
		/**
		 * In some versions, an empty string for a lore line is completely ignored, so
		 * at least a space " " is needed to get empty lore lines.
		 * <p>
		...
		private void handleTropicalFishBucketMeta(TropicalFishBucketMeta tropical) {
			...
			DyeColor patternColor = Enums.getIfPresent(DyeColor.class, config.getString("pattern-color"))
					.or(DyeColor.WHITE);
			TropicalFish.Pattern pattern = Enums.getIfPresent(TropicalFish.Pattern.class, config.getString("pattern"))
			...
			if (projectiles != null) {
				for (String projectile : projectiles.getKeys(false)) {
					ItemStack projectileItem = this.copySettings()
							.fromConfig(config.getConfigurationSection("projectiles." + projectile)).deserialize();
		...
		private void handleArmorMeta(ArmorMeta armor) {
			...
			if (trim != null) {
				TrimMaterial trimMaterial = Registry.TRIM_MATERIAL
						.get(NamespacedKey.fromString(trim.getString("material")));
				TrimPattern trimPattern = Registry.TRIM_PATTERN
						.get(NamespacedKey.fromString(trim.getString("pattern")));
		...
		private void handleFireworkMeta(FireworkMeta firework) {
			...
			for (String fws : fireworkSection.getKeys(false)) {
				...
				builder.with(
						Enums.getIfPresent(FireworkEffect.Type.class, fw.getString("type").toUpperCase(Locale.ENGLISH))
			...
			if (state instanceof CreatureSpawner) {
				...
				if (!Strings.isNullOrEmpty(spawnerStr)) {
					spawner.setSpawnedType(
							Enums.getIfPresent(EntityType.class, spawnerStr.toUpperCase(Locale.ENGLISH)).orNull());
					spawner.update(true);
				...
				if (shulkerSection != null) {
					...
					for (String key : shulkerSection.getKeys(false)) {
						ItemStack boxItem = this.copySettings().fromConfig(shulkerSection.getConfigurationSection(key))
								.deserialize();
				...
				if (patterns != null) {
					for (String pattern : patterns.getKeys(false)) {
						...
						if (patternType.isPresent() && patternType.get().isSupported()) {
							DyeColor color = Enums.getIfPresent(DyeColor.class,
									patterns.getString(pattern).toUpperCase(Locale.ENGLISH)).or(DyeColor.WHITE);
		...
		private static final boolean SUPPORTS_PotionMeta_setBasePotionType = XReflection.of(PotionMeta.class)
				.method("public void setBasePotionType(org.bukkit.potion.PotionType potionType)").exists();
		private static final boolean SUPPORTS_PotionEffectType_getKey = XReflection.of(PotionEffectType.class)
				.method("public org.bukkit.NamespacedKey getKey()").exists();
		...
		private void handlePotionMeta(ItemMeta meta) {
			if (supports(9)) {
				...
				// PotionType type = Enums.getIfPresent(PotionType.class,
				// split.get(0).trim().toUpperCase(Locale.ENGLISH)).or(PotionType.SLOWNESS);
				// boolean extended = split.size() != 1 &&
				// Boolean.parseBoolean(split.get(1).trim());
				// boolean splash = split.size() > 2 &&
				// Boolean.parseBoolean(split.get(2).trim());
				//
				// item = (splash ? XMaterial.SPLASH_POTION : XMaterial.POTION).parseItem();
				// PotionMeta potion = (PotionMeta) item.getItemMeta();
				// //
				// potion.addCustomEffect(XPotion.matchXPotion(type).buildPotionEffect(extended
				// ? 3 : 1, level), true);
		...
		private void handleSkullMeta(SkullMeta meta) {
			...
			if (skull != null) {
				...
				if (skull.isEmpty())
					XSkull.of(meta).profile(Profileable.detect(skull)).removeProfile();
				else
					XSkull.of(meta).profile(Profileable.detect(skull)).lenient().apply();
			}
		}
	 ...
	 * Deserialize an ItemStack from the config.
	 *
	 * @param config     the config section to deserialize the ItemStack object
	 *                   from.
	 * @param translator the function applied to item name and each lore line.
	 * @param restart    the function called when an error occurs while
	 *                   deserializing one of the properties.
	 * @return an edited ItemStack.
	 * @since 1.0.0
	 * @deprecated Use {@link Deserializer} instead.
	 */
	@NotNull
	@Deprecated
	public static ItemStack edit(@NotNull ItemStack item, @NotNull final ConfigurationSection config,
			@NotNull final Function<String, String> translator, @Nullable final Consumer<Exception> restart) {
		return new Deserializer().modifyItem(item).fromConfig(config).withTranslator(translator).withRestart(restart)
				.deserialize();
	}
	 ...
	 * Accepts the following formats: "r, g, b" "#RRGGBB" decimal number
	 * representing {@code "r << 16 | g << 8 | b"} (format "0xRRGGBB" is converted
	 * to decimal by SnakeYAML and handled as such)
	 *
	...
	public static Optional<Color> parseColor(@Nullable String str) {
		...
		// If we read a number that starts with 0x, SnakeYAML has already converted it
		// to base-10
		try {
	...
	private static <T> List<T> parseRawOrList(String singular, String plural, ConfigurationSection section,
	...
	/**
	 * Optimized version of {@link Inventory#addItem(ItemStack...)} <a href=
	 * "https://hub.spigotmc.org/stash/projects/SPIGOT/repos/craftbukkit/browse/src/main/java/org/bukkit/craftbukkit/inventory/CraftInventory.java">CraftInventory</a>
	 *
	 * @param inventory       the inventory to add the items to.
	 * @param split           false if it should check for the inventory stack size
	 *                        {@link Inventory#getMaxStackSize()} or true for item's
	 *                        max stack size {@link ItemStack#getMaxStackSize()}
	 *                        when putting items. This is useful when you're adding
	...
	@Contract(pure = true)
	public static List<ItemStack> stack(@NotNull Collection<ItemStack> items,
			@NotNull BiPredicate<ItemStack, ItemStack> similarity) {
	...
	/**
	 * Checks if this item is {@code null} or {@link Material#AIR}. The latter can
