...
public class AmountManager implements Listener {
	...
	void logDebug(String st) {
		...
		Logger.getLogger("Minecraft").log(Level.WARNING,
				"[DEBUG-ITEMREMOVAL][" + Main.getInstance().getName() + "]" + st);
	}
	...
	void handlesItemRemoval(InventoryClickEvent e, CraftingInventory inv, CraftingRecipeData recipe, ItemStack item,
		...
		if (matchesIngredient(item, recipeName, ingredient)) {
			...
			logDebug("[handleShiftClicks] PARAM_SLOT=" + slot + ", itemsToAdd=" + itemsToAdd + ", requiredAmount="
					+ requiredAmount + ", itemsToRemove_param=" + itemsToRemove);
			logDebug("[handleShiftClicks] ItemsToRemove CALC: " + itemsToRemove + " (itemsToAdd=" + itemsToAdd
					+ " * requiredAmount=" + requiredAmount + ")");
			logDebug("[handleShiftClicks] ItemsToAdd: " + itemsToAdd);
			logDebug("[handleShiftClicks] ItemAmount (available): " + availableItems + " || RequiredAmount: "
					+ requiredAmount);
			...
			if (availableItems < requiredAmount) {
				logDebug("[handleShiftClicks] REJECTED: availableItems (" + availableItems + ") < requiredAmount ("
						+ requiredAmount + ")");
				return;
			}
			...
			if (recipe.isLeftover(id)) {
				logDebug("[isLeftover] BRANCH=LEFTOVER: id=" + id + ", currentAmount=" + item.getAmount()
						+ ", requiredAmount=" + requiredAmount);
				...
				if (((item.getAmount()) - requiredAmount == 0)) {
					...
					if (containerCraft.get()) {
						...
						logDebug("[isLeftover-path2a] CONTAINER_EXACT_REMOVE: beforeAmount=" + item.getAmount()
								+ ", requiredAmount=" + requiredAmount + ", calculatedNew=" + newAmount);
						item.setAmount(newAmount);
						logDebug("[isLeftover-path2a] AFTER_SET: amount=" + item.getAmount());
						return;
					}
					int newAmount = (item.getAmount()) - (requiredAmount - 1);
					logDebug("[isLeftover-path2b] NORMAL_REMOVE_MINUS_1: beforeAmount=" + item.getAmount()
							+ ", requiredAmount=" + requiredAmount + ", calculatedNew=" + newAmount);
					item.setAmount(newAmount);
					logDebug("[isLeftover-path2b] AFTER_SET: amount=" + item.getAmount());
				}
			} else {
				logDebug("[NO_LEFTOVER] BRANCH=NORMAL: id=" + id + ", currentAmount=" + item.getAmount()
				...
				logDebug("[NO_LEFTOVER-path2] PARTIAL_REMOVE: beforeAmount=" + item.getAmount() + ", toRemove="
						+ itemsToRemove);
				item.setAmount(item.getAmount() - (itemsToRemove - 1));
	...
	private void handleResultClickInternal(InventoryClickEvent e, boolean sourceIsCraftItemEvent) {
		...
		// Guard against duplicate result-click events fired for the same physical
		// click.
		...
		try {
			...
			logDebug(
					"[handleShiftClicks] Failed to read result NBT due to invalid stack. Continuing with fallback recipe name.");
		}
		...
		if (types != null)
			for (Recipe r : types.values()) {
				...
				if (r.getType() != RecipeType.SHAPED && r.getType() != RecipeType.SHAPELESS)
					continue;
		...
		Recipe rawRecipe = getRecipeUtil().getRecipe(findName);
		if (rawRecipe == null || !(rawRecipe instanceof CraftingRecipeData))
			return;

		CraftingRecipeData recipe = (CraftingRecipeData) rawRecipe;
		...
		Bukkit.getScheduler().runTaskLater(Main.getInstance(), () -> Main.getInstance().getInInventory().remove(id),
				2L);
	...
	public void traceResultSlotClicks(InventoryClickEvent e) {
		...
		logDebug("[traceResultSlotClicks] class=" + e.getClass().getSimpleName() + ", action=" + e.getAction()
				+ ", click=" + e.getClick() + ", cancelled=" + e.isCancelled() + ", current=" + cur + ", invResult="
				+ res + ", invRecipe=" + recipe);
